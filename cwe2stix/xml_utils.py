...
def parse_description(weakness_el: Element):
    description = getTextFromNode(firstOrNone(weakness_el.getElementsByTagName('Description')))
    if extended_el := firstOrNone(weakness_el.getElementsByTagName('Extended_Description')):
        buf = io.StringIO()
        extended_el.writexml(buf)
        buf.seek(0)
        extended_description = buf.getvalue().replace('xhtml:', '').replace("Extended_Description>", "span>")
